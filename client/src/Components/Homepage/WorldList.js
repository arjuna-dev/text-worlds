...
function WorldList() {
  ...
  const allWorldData = data;

  if (loading) {
    return (
      <div className="ui active centered loader loading-screen">Loading..</div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="world-list">
        ...
        <List size="massive">
          {allWorldData.worlds.map((world) => (
            <List.Item
              style={{ marginBottom: "9vh" }}
              key={world._id}
              data-testid={`world-item-${world._id}`}
            >
              <a href={"/world/" + world._id}>
                <Image
                  style={{ display: "inline-block" }}
                  avatar
                  src={picNames[Math.floor(Math.random() * Math.floor(4))]}
                  size="small"
                />
              </a>
              <List.Content
              ...
              >
                <a href={"/world/" + world._id}>
                  <List.Header style={{ marginBottom: "1vh" }}>
                    <div className="world-header" data-testid="world-header">
                      {world.name}
                    </div>
                  </List.Header>
                </a>
                <List.Description>
                  ...
                  >
                    <ReadMore lines={2}>{world.description}</ReadMore>
                  </Segment>
                </List.Description>
              </List.Content>
            </List.Item>

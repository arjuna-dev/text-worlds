...
const ModalPopup = (props) => {
  ...
  let link = "/world/" + props.worldDetailsData.world._id + "/graph";
  ...
  const handleSubmit = (e) => {
    ...
    addCharacter({
      variables: {
        name: name,
        story: story,
        worldId: props.worldDetailsData.world._id,
        userId: jwt_decode(localStorage.usertoken)._id,
      },
      refetchQueries: [
        {
          query: getWorldQuery,
          variables: { id: props.worldDetailsData.world._id },
        },
      ],
    }).then(() => {
      setName("");
  ...
  if (!localStorage.usertoken) {
    return (
      <Modal
        size="mini"
        trigger={
          <Button neutral="true" className="join-world">
            Join the world
          </Button>
        }
      >
  ...
  if (props.userAlreadyJoined) {
    return (
      <div>
        {" "}
        <Link to={link}>
          <Button positive className="join-world">
            Enter the world
          </Button>
        </Link>
      </div>
  ...
  return (
    ...
    >
      <Modal.Header className="add-character-form">
